...
Here, we begin with two patsy ideas: one that miles/gallon (mpg) goes up as car engine size (capacity) goes down **and** as car weight goes down. Our contrasting theory predict that it is "*only weight matters*", not engine capacity. We can drop capacity to test the idea that the only determinant (in this confounded play dataset) of mpg is weight.
