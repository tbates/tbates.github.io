...
*note*: This double-step was required because OpenMx used to over-write the `X→Y` path when you add `X↔Y`.
The GenomicMx fork of OpenMx does not do this any more.
...
## umxMR
Now the equivalent model in `umxMR`:
