...
```splus
m1 = sem::tsls(formula = Y ~ X, instruments = ~ qtl, data = MR_data); coef(m1)
...
m2 <- umxRAM("myMR", data = MR_data, autoRun = F,
...
SB <- umxRAM("SB_IV", data = mxData(MR_data, type="raw"),
