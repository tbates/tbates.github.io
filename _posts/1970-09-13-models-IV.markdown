...
```
*note*: This double-step is required because OpenMx will over-write the `X→Y` path when you add `X↔Y`.
...
Now an exactly equivalent model in `umxTSLS`/ `umxMendelianRandomization`
